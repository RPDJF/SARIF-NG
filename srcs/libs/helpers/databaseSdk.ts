...
export const defaultConfig: DatabaseSdkConfig = {
	...
	serverUrl: process.env.DATABASE_URL || "https://sarif-ng-database:3000",
};
...
export default class DatabaseSDK {
	...
	private async api_request<T>(
		...
		table: "Players" | "Matches" | "Tournaments" | "CurrentContract",
		endpoint?: string,
		options?: db_sdk_options
	...
	private async validate_match(val_match: Match): Promise<comb> {
		...
		if (val_match.HashAddress) {
			await this.bc_sdk
				.get_match_score(val_match.MatchID as string)
				.then(function (response) {
					merged = { ...val_match, ...response };
				})
				.catch(function () {
					val_match.HashAddress = undefined;
				});
		}
		if (merged) {
		...
		} else merged = val_match;
		const uarray: User[] = [];
		try {
			uarray[0] = this.usr_sdk.filterPublicUserData(
				(await this.usr_sdk.getUser(merged.WPlayerID as string)).data
			) as User;
		} catch (exception) {
		...
		try {
			uarray[1] = this.usr_sdk.filterPublicUserData(
				(await this.usr_sdk.getUser(merged.LPlayerID as string)).data
			) as User;
		} catch (exception) {
			if (exception instanceof AxiosError && exception.status === 404)
	...
	private async get_player_matchlist_from_uuid(
		user: UUIDv4
	): Promise<Array<Match_complete>> {
		const matchlist: Array<Match> = await this.api_request<Array<Match>>(
			"GET",
			"Matches",
			`/PlayerID/${this.param_str}`,
			{ params: user }
	...
	private async get_player_matchlist_from_user(
		user: User
	): Promise<Array<Match_complete>> {
		if (!user.PlayerID) throw "error.missing.playerid";
		return (await this.get_player_matchlist_from_uuid(
			user.PlayerID
		)) as Array<Match_complete>;
	}

	private async get_friends_from_user(user: User): Promise<Array<User>> {
		if (!user.FriendsList) return [];
		const ret: Array<User> = await this.api_request<Array<User>>(
			"GET",
			"Players",
			"/multiget",
			{
	...
	public async create_user(user: User): Promise<AxiosResponse<User>> {
		return await this.api_request<User>("POST", "Players", undefined, {
			body: user,
		});
	}
	...
	public async get_user(
		identifier: string,
		type: "PlayerID" | "DisplayName" | "EmailAddress" | "OAuthID"
	): Promise<AxiosResponse<User>> {
		return await this.api_request<User>(
			"GET",
			"Players",
			`/${type}/${this.param_str}`,
			{ params: identifier }
	...
	public async update_user(user: Partial<User>): Promise<AxiosResponse<User>> {
		...
		return await this.api_request<User>(
			...
			"Players",
			`/PlayerID/${this.param_str}`,
			{ body: user, params: user.PlayerID }
		);
	}
	...
	public async delete_user(user: UUIDv4): Promise<AxiosResponse<string>> {
		return await this.api_request<string>(
			"DELETE",
			"Players",
			`/PlayerID/${this.param_str}`,
			{ params: user }
	...
	public async get_user_friends(user: UUIDv4 | User): Promise<Array<User>> {
		if (typeof user === "string") return await this.get_friends_from_uuid(user);
	 ...
	 */
	public async get_user_picture(
		identifier: UUIDv4
	): Promise<AxiosResponse<ArrayBuffer>> {
		return await this.api_request<ArrayBuffer>(
			"GET",
			"Players",
			`/PlayerID/${this.param_str}/picture`,
			{ params: identifier, response_type: "arraybuffer" }
		);
	}
	...
	public async set_user_picture(
		...
		return await this.api_request<ArrayBuffer>(
			...
			"Players",
			`/PlayerID/${this.param_str}/picture`,
			{ params: identifier, body: picture, response_type: "arraybuffer" }
		);
	}
	 ...
	 */
	public async log_user(
		identifier: string,
		type: "PlayerID" | "DisplayName" | "EmailAddress",
		plaintext: string
	): Promise<AxiosResponse<User>> {
		return await this.api_request<User>(
			"GET",
			"Players",
			`/${type}/${this.param_str}/check_passwd`,
			{
				params: identifier,
				headers: {
					Password: plaintext,
	...
	public async create_tournament(tournament: Tournament_metadata) {
		return await this.api_request<Tournament_metadata>(
			"POST",
			"Tournaments",
			undefined,
			{ body: tournament }
	 ...
	 */
	public async get_tournament(
		tournament_id: UUIDv4
	): Promise<AxiosResponse<Tournament_full>> {
		return await this.api_request<Tournament_full>(
			"GET",
			"Tournaments",
			`/TournamentID/${this.param_str}`,
	 ...
	 * @returns An AxiosResponse Promise containing an array of every single Match.
	 */
	public async get_player_matchlist(
		user: UUIDv4 | User
	): Promise<Array<Match_complete>> {
	...
	public async create_match(match: Match): Promise<AxiosResponse<Match>> {
		const finished_match: Match = await this.api_request<Match>(
			"POST",
			"Matches",
			undefined,
			{ body: match }
		...
		const match_tx: TXHash = await this.bc_sdk
			.add_match_score(finished_match)
			.then((response) => response.data)
			.catch(() => {
				throw "error.blockchain.down";
			});
		return await this.api_request<Match>(
			"PUT",
			"Matches",
			`/MatchID/${this.param_str}`,
			{ body: { HashAddress: match_tx }, params: finished_match.MatchID }
		);
	}
	...
	public async get_match(match_id: UUIDv4): Promise<Match_complete> {
		const match: Match = await this.api_request<Match>(
			"GET",
			"Matches",
			`/MatchID/${this.param_str}`,
			{ params: match_id }
